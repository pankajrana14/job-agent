...
Warm amber professional dark design.
...
# ── Warm amber professional palette ───────────────────────────────────────────
_C = {
    "bg":        "#0B0906",          # very dark warm black — sidebar base
    "surface":   "#131109",          # content area surface
    "surface2":  "#1E1B14",          # input fields — visible lift over surface
    "surface3":  "#252118",          # card/groupbox backgrounds
    "border":    "rgba(210,195,165,0.07)",
    "border_h":  "rgba(210,195,165,0.14)",
    "border_s":  "rgba(210,195,165,0.10)",   # card top highlight
    "text":      "#E8E3DA",          # warm off-white
    "text2":     "#9A9182",          # secondary
    "text3":     "#5C5549",          # muted
    "accent":    "#8C7A52",          # muted warm bronze
    "accent2":   "#A08E68",          # lighter bronze
    "accent_bg": "rgba(140,122,82,0.09)",
    "accent_b":  "rgba(140,122,82,0.20)",
    "green":     "#4aab6d",
    "green_bg":  "rgba(74,171,109,0.09)",
    "green_b":   "rgba(74,171,109,0.25)",
    "red":       "#c96060",
    "red_bg":    "rgba(201,96,96,0.09)",
    "red_b":     "rgba(201,96,96,0.25)",
}
...
_CHK_FILE.write_text(
    ...
    '<polyline points="3,9.5 7,13.5 15,4.5" stroke="#0E0C0A" stroke-width="2.5"'
...
def _ui_font() -> str:
    ...
    for f in ("Segoe UI Variable", "Segoe UI"):
        if f in families:
            return f
    return "Arial"
...
def _make_qss(font: str) -> str:
    c = _C
    return f"""
* {{ font-family: "{font}", "Segoe UI", Arial, sans-serif; font-size: 14px; }}
QMainWindow {{ background: transparent; }}
QWidget      {{ background: transparent; color: {c['text']}; }}

/* ══ SIDEBAR — very dark, visually distinct ══ */
QFrame#sidebar {{
    background: {c['bg']};
    border-right: 1px solid rgba(235,215,170,0.06);
}}
QWidget#sidebar_accent {{
    background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
        stop:0 rgba(140,122,82,0.55), stop:0.5 rgba(140,122,82,0.18), stop:1 transparent);
    min-height: 2px; max-height: 2px;
}}
QLabel#app_name {{
    color: {c['accent2']}; font-size: 18px; font-weight: 700;
    background: transparent; letter-spacing: -0.5px;
}}
QLabel#app_tag {{
    color: {c['text3']}; font-size: 11px; background: transparent; letter-spacing: 0.2px;
}}

/* ── Nav buttons — rounded, commercial ── */
QPushButton#nav_btn {{
    background: transparent; border: none; border-radius: 8px;
    color: {c['text3']}; text-align: left; padding: 10px 16px;
    font-size: 13.5px; font-weight: 400; margin: 1px 8px;
}}
QPushButton#nav_btn:hover {{
    background: rgba(235,215,170,0.06); color: {c['text2']};
}}
QPushButton#nav_btn:focus {{ outline: none; color: {c['text2']}; }}
QPushButton#nav_btn[active=true] {{
    background: {c['accent_bg']}; color: {c['accent']};
    font-weight: 600; border: 1px solid {c['accent_b']};
}}
...
QWidget#content_bg {{
    ...
    border-left: 1px solid rgba(235,215,170,0.06);
}}
...
QScrollBar::handle:vertical {{
    background: rgba(235,215,170,0.12); border-radius: 3px; min-height: 28px;
}}
QScrollBar::handle:vertical:hover {{ background: {c['accent_b']}; }}
...
QLabel#page_desc  {{
    font-size: 13px; color: {c['text2']}; background: transparent;
}}
QLabel {{ color: {c['text2']}; background: transparent; }}

/* ── Run page labels ── */
QLabel#path_info  {{ font-size: 11px; color: {c['text3']}; background: transparent; }}
QLabel#output_lbl {{ color: {c['text3']}; font-size: 10px; font-weight: 600; letter-spacing: 0.8px; background: transparent; }}
QLabel#status_lbl {{ font-size: 13px; font-weight: 500; color: {c['text3']}; background: transparent; }}

/* ══ CARDS / GROUP BOXES — visible depth ══ */
QGroupBox {{
    background: {c['surface3']};
    border: 1px solid {c['border']};
    border-top: 1px solid {c['border_s']};
    border-radius: 10px;
    margin-top: 16px; padding: 18px 18px 20px 18px;
}}
QGroupBox:hover {{ border-color: rgba(140,122,82,0.14); border-top-color: rgba(140,122,82,0.20); }}
QGroupBox::title {{
    subcontrol-origin: margin; subcontrol-position: top left;
    left: 16px; padding: 0 8px;
    color: rgba(160,142,104,0.50); background: {c['surface3']};
...
QLineEdit {{
    ...
    border-radius: 7px; padding: 9px 13px; color: {c['text']};
    selection-background-color: {c['accent']}; selection-color: {c['bg']};
}}
QLineEdit:hover {{ border-color: {c['border_h']}; }}
QLineEdit:focus {{ border-color: rgba(140,122,82,0.50); outline: none; }}

QPlainTextEdit, QTextEdit {{
    background: {c['surface2']}; border: 1px solid {c['border']};
    border-radius: 7px; padding: 10px 13px; color: {c['text']};
    selection-background-color: {c['accent']}; selection-color: {c['bg']};
}}
QPlainTextEdit:focus, QTextEdit:focus {{ border-color: rgba(140,122,82,0.50); outline: none; }}

QTextEdit#output_log {{
    background: rgba(9,8,5,0.98); border: 1px solid {c['border']};
    border-radius: 7px; padding: 10px; color: {c['text2']};
}}

QSpinBox, QDoubleSpinBox {{
    background: {c['surface2']}; border: 1px solid {c['border']};
    border-radius: 7px; padding: 8px 10px; color: {c['text']}; min-width: 90px;
}}
QSpinBox:focus, QDoubleSpinBox:focus {{ border-color: rgba(140,122,82,0.50); outline: none; }}
QSpinBox::up-button, QDoubleSpinBox::up-button,
QSpinBox::down-button, QDoubleSpinBox::down-button {{
    background: rgba(235,215,170,0.06); border: none; width: 18px; border-radius: 4px;
}}
...
QCheckBox::indicator {{
    ...
    border: 1px solid rgba(235,215,170,0.20); border-radius: 5px;
    background: {c['surface2']};
}}
QCheckBox::indicator:hover {{ border-color: rgba(140,122,82,0.44); }}
QCheckBox:focus {{ outline: none; }}
QCheckBox::indicator:focus {{ border-color: rgba(140,122,82,0.50); }}
QCheckBox::indicator:checked {{ background: {c['accent']}; border-color: {c['accent']}; image: url("{_CHK}"); }}

QListWidget {{
    background: {c['surface2']}; border: 1px solid {c['border']};
    border-radius: 7px; padding: 4px; color: {c['text']}; outline: none;
    alternate-background-color: rgba(26,23,16,0.70);
}}
QListWidget:focus {{ border-color: rgba(140,122,82,0.35); outline: none; }}
QListWidget::item {{ padding: 6px 10px; border-radius: 5px; }}
QListWidget::item:selected {{
    background: {c['accent_bg']}; color: {c['accent']}; border: 1px solid {c['accent_b']};
}}
QListWidget::item:hover {{ background: rgba(235,215,170,0.05); }}
...
QPushButton {{
    ...
    color: {c['text']};
    border: 1px solid rgba(235,215,170,0.11);
    border-top: 1px solid rgba(235,215,170,0.22);
    border-radius: 7px;
    padding: 8px 18px; font-size: 14px; font-weight: 500; min-height: 32px;
}}
QPushButton:hover {{
    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
        stop:0 rgba(255,255,255,0.13), stop:1 rgba(255,255,255,0.05));
    border-color: rgba(235,215,170,0.18); border-top-color: rgba(235,215,170,0.32);
}}
QPushButton:focus {{ border-color: rgba(140,122,82,0.42); outline: none; }}
QPushButton:pressed {{ background: rgba(235,215,170,0.03); border-color: rgba(235,215,170,0.08); }}
QPushButton:disabled {{ background: rgba(18,15,11,0.50); color: {c['text3']}; border-color: rgba(235,215,170,0.04); }}

/* Override base for nav_btn */
QPushButton#nav_btn {{
    background: transparent; border: none; border-radius: 8px;
...
QPushButton#save_btn {{
    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
        stop:0 #957D4E, stop:1 #7A6438);
    color: #0D0B08; border: 1px solid rgba(140,122,82,0.28);
    border-top: 1px solid rgba(200,180,140,0.32);
    font-weight: 600; border-radius: 8px; letter-spacing: 0.1px;
}}
QPushButton#save_btn:hover {{ background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #A08A58, stop:1 #856D42); }}
QPushButton#save_btn:focus {{ border-color: rgba(200,180,140,0.55); outline: none; }}
QPushButton#save_btn:pressed {{ background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #7A6438, stop:1 #6A5530); }}
...
QPushButton#run_btn {{
    ...
    border-radius: 9px; padding: 10px 36px; letter-spacing: 0.2px;
}}
QPushButton#run_btn:hover {{ background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #A08A58, stop:1 #856D42); }}
QPushButton#run_btn:focus {{ border-color: rgba(200,180,140,0.55); outline: none; }}
QPushButton#run_btn:pressed {{ background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #7A6438, stop:1 #6A5530); }}
QPushButton#run_btn:disabled {{ background: rgba(25,22,16,0.70); color: {c['text3']}; border-color: rgba(210,195,165,0.04); }}

QPushButton#stop_btn {{
    background: {c['red_bg']};
    color: {c['red']}; border: 1px solid {c['red_b']};
    border-top: 1px solid rgba(201,96,96,0.35);
    font-weight: 600; min-height: 46px; border-radius: 9px;
}}
QPushButton#stop_btn:hover {{ background: rgba(201,96,96,0.14); }}
QPushButton#stop_btn:pressed {{ background: rgba(201,96,96,0.20); }}
QPushButton#stop_btn:focus {{ border-color: rgba(201,96,96,0.50); outline: none; }}
QPushButton#stop_btn:disabled {{ color: {c['text3']}; border-color: {c['border']}; background: transparent; }}

QPushButton#add_btn {{
    background: {c['green_bg']};
    color: {c['green']}; border: 1px solid {c['green_b']};
    border-radius: 6px; padding: 5px 14px; font-size: 13px; min-height: 26px; font-weight: 600;
}}
QPushButton#add_btn:hover {{ background: rgba(74,171,109,0.14); }}
QPushButton#add_btn:pressed {{ background: rgba(74,171,109,0.22); }}
QPushButton#add_btn:focus {{ border-color: rgba(74,171,109,0.50); outline: none; }}

QPushButton#rem_btn {{
    background: transparent; color: {c['text3']};
    border: 1px solid rgba(235,215,170,0.08);
    border-radius: 6px; padding: 5px 14px; font-size: 13px; min-height: 26px;
}}
QPushButton#rem_btn:hover {{ color: {c['red']}; border-color: {c['red_b']}; background: {c['red_bg']}; }}
QPushButton#rem_btn:focus {{ border-color: rgba(235,215,170,0.25); outline: none; }}

QPushButton#clear_btn {{
    background: transparent; color: {c['text3']};
    border: none; padding: 5px 10px; font-size: 13px; min-height: 26px;
}}
QPushButton#clear_btn:hover {{ color: {c['text2']}; }}
QPushButton#clear_btn:focus {{ color: {c['text2']}; outline: none; }}

/* ── Dividers ── */
QFrame[frameShape="4"] {{
    border: none; border-top: 1px solid rgba(235,215,170,0.07);
    max-height: 1px; background: rgba(235,215,170,0.07);
}}

QToolTip {{
    background: {c['surface3']}; color: {c['text']};
    border: 1px solid {c['border_h']}; border-radius: 6px;
...
def _make_icon() -> QIcon:
    ...
    for size in (16, 32, 48, 64, 128, 256):
        ...
        pen2 = QPen(QColor(_C["bg"]), max(1.0, s*0.06))
...
class AnimatedButton(QPushButton):
    ...
    def paintEvent(self, e):
        ...
        if self._op > 0.001:
            ...
            # Primary amber buttons get a warm glow; others get a neutral lift
            name = self.objectName()
            if name in ("run_btn", "save_btn"):
                c = QColor(200, 175, 120); c.setAlphaF(self._op * 0.09)
            else:
                c = QColor(210, 195, 165); c.setAlphaF(self._op * 0.05)
            path = QPainterPath()
            r = 8.0 if name in ("run_btn", "stop_btn", "save_btn") else 6.0
...
class _EmptyListWidget(QListWidget):
    """QListWidget that shows placeholder text when empty."""

    def __init__(self, placeholder: str = "", *a, **kw) -> None:
        super().__init__(*a, **kw)
...
def _header(title: str, desc: str) -> QWidget:
    ...
    lay.setContentsMargins(0, 0, 0, 4); lay.setSpacing(4)
    t = QLabel(title); t.setObjectName("page_title")
    d = QLabel(desc);  d.setObjectName("page_desc")
    # Amber-tinted divider — more premium than plain grey line
    div = QWidget(); div.setFixedHeight(1)
    div.setStyleSheet(
        "background: qlineargradient(x1:0, y1:0, x2:1, y2:0,"
        f" stop:0 rgba(140,122,82,0.45), stop:0.35 rgba(140,122,82,0.14), stop:1 transparent);"
    )
    div.setAttribute(Qt.WA_StyledBackground, True)
    lay.addWidget(t); lay.addWidget(d); lay.addSpacing(8); lay.addWidget(div)
    return w
...
class CredentialsPage(QWidget):
    ...
    def _build(self):
        ...
        em = QGroupBox("EMAIL / SMTP"); ef = QFormLayout(em)
        ef.setVerticalSpacing(10); ef.setHorizontalSpacing(16)
        ef.setLabelAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        ...
        k = QGroupBox("LLM API KEYS"); kf = QFormLayout(k)
        kf.setVerticalSpacing(10); kf.setHorizontalSpacing(16)
        kf.setLabelAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._field(kf, "OPENROUTER_API_KEY", "OpenRouter", True)
        self._field(kf, "GROQ_API_KEY", "Groq (optional)", True)
        lay.addWidget(k)

        s = QGroupBox("LLM SETTINGS"); sf = QFormLayout(s)
        sf.setVerticalSpacing(10); sf.setHorizontalSpacing(16)
        sf.setLabelAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._field(sf, "LLM_MODEL", "Model  (e.g. openrouter/openai/gpt-oss-120b:free or groq/openai/gpt-oss-120b)", False)
...
class ConfigPage(QWidget):
    ...
    def _build(self):
        root = QVBoxLayout(self); root.setContentsMargins(32, 24, 32, 20); root.setSpacing(0)
        root.addWidget(_header("Configuration",
            "Search queries, platform toggles, and scraping parameters"))
        root.addSpacing(16)
        inner = QWidget(); lay = QVBoxLayout(inner); lay.setContentsMargins(0, 0, 10, 0); lay.setSpacing(12)

        pf = QGroupBox("CANDIDATE PROFILE  (PROFILE.md)"); pl = QVBoxLayout(pf)
        self._prof.setPlaceholderText("Paste your CV, skills, and preferences here.\nThe AI uses this to score each job's fit.")
        self._prof.setMinimumHeight(150); pl.addWidget(self._prof); lay.addWidget(pf)

        loc = QGroupBox("SEARCH LOCATION"); lf = QFormLayout(loc)
        lf.setVerticalSpacing(10); lf.setHorizontalSpacing(16)
        lf.setLabelAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self._country.setPlaceholderText("e.g. Germany, Netherlands, United Kingdom (LinkedIn only)")
        self._country.setToolTip(
            "Country passed to LinkedIn's location filter.\n"
            "StepStone supports: Germany, Austria, Belgium, Netherlands.\n"
            "BA Jobbörse is Germany-only and is skipped for other countries."
        )
        lf.addRow("Country", self._country)
        note = QLabel("BA Jobbörse is Germany-only — it is skipped automatically for other countries.")
        note.setObjectName("path_info"); note.setWordWrap(True)
        lf.addRow(note)
        lay.addWidget(loc)
    ...
    def _load(self):
        ...
        self._country.setText(self._s(t, "SEARCH_COUNTRY", "Germany"))
    ...
    def _s(t, v, d):
        ...
        if not m:
            return d
        g1 = m.group(1)
        return g1 if g1 is not None else m.group(2)
    @staticmethod
    def _lst(t, v):
...
class RunPage(QWidget):
    ...
    def _build(self):
        # Run page gets more generous margins — it's the primary action page
        root = QVBoxLayout(self); root.setContentsMargins(40, 36, 40, 28); root.setSpacing(0)
        root.addWidget(_header("Run Pipeline", "Start a full search run — results arrive in your inbox"))
        root.addSpacing(24)

        info = QLabel(
            f"<b>Python:</b> <span style='font-family:Consolas'>{_PYTHON}</span><br>"
            f"<b>Working dir:</b> <span style='font-family:Consolas'>{_PROJECT_DIR}</span>")
        info.setObjectName("path_info")
        info.setWordWrap(True)
        root.addWidget(info); root.addSpacing(24)
        ...
        root.addLayout(row); root.addSpacing(14)

        self._status = QLabel("")
        self._status.setObjectName("status_lbl")
        root.addWidget(self._status); root.addSpacing(12)

        out_row = QHBoxLayout()
        lbl = QLabel("OUTPUT"); lbl.setObjectName("output_lbl")
        ...
        self._out.setObjectName("output_log")
    ...
    def _do_run(self):
        ...
        self._status.setStyleSheet(f"color:{_C['accent']};")
    ...
    def _do_stop(self):
        ...
        self._status.setStyleSheet(f"color:{_C['red']};")
    ...
    def _done(self, code):
        ...
        if code == 0:
            ...
            self._status.setStyleSheet(f"color:{_C['green']};")
        else:
            self._status.setText(f"Failed  (exit code {code})")
            self._status.setStyleSheet(f"color:{_C['red']};")

    def _clear(self): self._out.clear(); self._status.clear(); self._status.setStyleSheet("")
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        sb_frame.setFixedWidth(220); sb_frame.setAttribute(Qt.WA_StyledBackground, True)
        sb = QVBoxLayout(sb_frame); sb.setContentsMargins(0, 0, 0, 0); sb.setSpacing(0)

        # Amber accent strip at very top — brand mark
        accent_strip = QWidget(); accent_strip.setObjectName("sidebar_accent")
        accent_strip.setAttribute(Qt.WA_StyledBackground, True)
        accent_strip.setFixedHeight(2)
        sb.addWidget(accent_strip)

        # Header — brand area
        hdr = QWidget(); hdr.setAttribute(Qt.WA_StyledBackground, True)
        hdr.setStyleSheet("background:transparent;")
        hl = QVBoxLayout(hdr); hl.setContentsMargins(20, 22, 20, 20); hl.setSpacing(4)
        ico = QLabel(); ico.setPixmap(_make_icon().pixmap(34, 34)); ico.setStyleSheet("background:transparent;")
        hl.addWidget(ico); hl.addSpacing(10)
        nm = QLabel("Job Agent"); nm.setObjectName("app_name")
        tg = QLabel("AI-powered job search"); tg.setObjectName("app_tag")
        hl.addWidget(nm); hl.addWidget(tg)
        sb.addWidget(hdr); sb.addWidget(_hline()); sb.addSpacing(8)
